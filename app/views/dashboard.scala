...
object dashboard {
  ...
  views.main.layout("Dashboard", frag(), frag(content(currentUser, currentUserRights, applications, groups, filters, config)))
  ...
  div()(
    ...
    p(cls := "aplus-dashboard-date")(s"aujourd’hui ${java.time.LocalDate.now().format(java.time.format.DateTimeFormatter.ofPattern("dd MMM yyyy"))}"),
    div(cls := "fr-grid-row fr-grid-row--gutters fr-mb-1w")(
      div(cls := "fr_card__outer_container fr-col-md-6 fr-col")(
        div(cls := "fr_card fr-enlarge-link fr-card--horizontal fr-card--horizontal-half fr_card__highlight") (
      ...
      div(cls := "fr_card__outer_container fr-col-md-12 fr-col")(
        div(cls := "fr_card fr-enlarge-link fr-card--horizontal") (
          // display table only if there is at least one group in groups
          if(groups.nonEmpty)(
