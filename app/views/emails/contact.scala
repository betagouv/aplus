...
import controllers.routes.{Assets, HomeController}
  ...
  def page(config: AppConfig): Tag =
    views.main.publicLayout(
      ...
      div(cls := "fr-mb-15w")(
        ...
        div(cls := "fr-grid-row fr-mt-6w")(
          div(cls := "fr-col-12")(
            div(cls := "fr-highlight")(
              div(cls := "fr-mb-1w aplus-text--blue")(
                span(cls := "fr-icon-question-answer-fill", aria.hidden := "true")
              ),
              h3(cls := "fr-text--lead")("Nouveau : échangez avec l’équipe d’A+ par tchat !"),
              p(cls := "fr-mb-1w")(
                "Si le message ci-dessous est visible en bas à droite de votre navigateur, cliquez dessus pour dialoguer avec notre équipe."
              ),
              img(
                src := Assets.versioned("/images/zammad-chat.png").url,
                attr("srcset") :=
                  s"${Assets.versioned("/images/zammad-chat.png").url} 1x, ${Assets.versioned("/images/zammad-chat_2x.png").url} 2x",
                alt := "Image du tchat Zammad"
              ),
              p(cls := "fr-text--sm fr-mt-2w")(
                "Outil en phase de test. Les horaires de disponibilité ne sont pas fixés pour le moment."
              )
            )
          )
        )
