...
object User {
  ...
  val admins =  List(
   ...
   // Disabled
    User(UUIDHelper.namedFrom("simon"), Hash.sha256(s"simon - disabled"), "Simon PINEAU", "Expert A+ (disabled)", "simon.pineau@beta.gouv.fr - disabled", false, false, true, Area.all.map(_.id), date, false, "75056", true, expert = true, cguAcceptationDate = Some(date)),
    User(UUIDHelper.namedFrom("louis"), Hash.sha256(s"louis - disabled"), "Louis MOSCAROLA (disabled)", "Expert A+", "louis.moscarola@beta.gouv.fr - disabled", false, false, false, List(), date, false, "75056", false),
