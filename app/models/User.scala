...
object User {
  ...
  val admins =  List(
    ...
    User(UUIDHelper.namedFrom("julien"), Hash.sha256(s"julien"), "Julien DAUPHANT", "Admin A+", "julien.dauphant@beta.gouv.fr", true, false, true, Area.all.map(_.id), date, true, "75056", true),
    User(UUIDHelper.namedFrom("louis"), Hash.sha256(s"louis"), "Louis MOSCAROLA", "Expert A+", "louis.moscarola@beta.gouv.fr", true, false, true, Area.all.map(_.id), date, true, "75056", true, expert = true),
    User(UUIDHelper.namedFrom("sylvain"), Hash.sha256(s"sylvain"), "Sylvain DERMY", "Expert A+", "sylvain.dermy@beta.gouv.fr", true, false, true, Area.all.map(_.id), date, true, "75056", true, expert = true),
    // Disabled
    ...
    User(UUIDHelper.namedFrom("simon"), Hash.sha256(s"simon - disabled"), "Simon PINEAU (disabled)", "Aide A+", "simon.pineau@beta.gouv.fr - disabled", true, false, false, List(), date, false, "75056", false),
    User(UUIDHelper.namedFrom("dominique"), Hash.sha256(s"dominique - disabled"), "Dominique LEQUEPEYS (disabled)", "Aide A+", "dominique.lequepeys@beta.gouv.fr - disabled", false, false, false, List(), date, false, "75056", false),
  )
}
