...
object User {
  ...
  val admins =  List(
    ...
    User(UUIDHelper.namedFrom("simon"), Hash.sha256(s"simon"), "Simon PINEAU", "Expert A+", "simon.pineau@beta.gouv.fr", true, false, true, Area.all.map(_.id), date, false, "75056", true, disabled = false, expert = true, cguAcceptationDate = Some(date)),
   // Disabled
