...
import models.EventType.{AddExpertCreated, AgentsAdded, AllApplicationsShowed, AllAsShowed, AllCSVShowed, AnswerCreated, ApplicationCreated, ApplicationCreationInvalid, ApplicationFormShowed, ApplicationShowed, FileOpen, MyApplicationsShowed, MyCSVShowed, StatsShowed, TerminateCompleted}
    ...
    (answerIdOption,applicationService.byId(applicationId, request.currentUser.id, request.currentUser.admin)) match {
      ...
      case (Some(answerId), Some(application)) if application.fileCanBeShowed(request.currentUser, answerId) =>
          application.answers.find(_.id == answerId) match {
            case Some(answer) if answer.files.getOrElse(Map()).contains(filename) =>
              eventService.log(FileOpen, s"Le fichier de la réponse $answerId sur la demande $applicationId a été ouvert")
      ...
      case (None, Some(application)) if application.fileCanBeShowed(request.currentUser) =>
        if(application.files.contains(filename)) {
            eventService.log(FileOpen, s"Le fichier de la demande $applicationId a été ouvert")
