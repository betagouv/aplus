    ...
    if(request.currentUser.admin == false) {
      ...
      def userToCSV(user: User): String = {
        List[String](
          ...
          user.qualite,
          user.email,
          user.creationDate.toString("dd-MMM-YYY-HHhmm", new Locale("fr")),
          ...
          if(user.cguAcceptationDate.nonEmpty) { "CGU Accepté" } else { "" },
          if(user.newsletterAcceptationDate.nonEmpty) { "Newsletter Accepté"} else { "" },
        ).mkString(";")
      }
