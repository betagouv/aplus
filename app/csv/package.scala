...
package object csv {
  ...
  val USER_EMAIL = Header(
    "user.email",
    List("Email", "Adresse e-mail", "Contact mail Agent", "MAIL")
  )
  ...
  val USER_PHONE_NUMBER =
    Header("user.phone-number", List("Numéro de téléphone", "téléphone"))

  val GROUP_AREAS_IDS =
    Header("group.area-ids", List("Territoire", "DEPARTEMENTS"))
