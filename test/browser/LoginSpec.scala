...
class LoginSpec extends Specification with Tables with BaseSpec {
  ...
  "Login" should {
    "Login with valid or invalid emails" in new WithBrowser(webDriver = WebDriverFactory(HTMLUNIT), app = applicationWithBrowser) {
        ...
        "wrong@beta.gouv.fr"                 ! "Aucun compte actif à cette adresse email"           |
        "simon.pineau" + "@beta.gouv.fr"     ! "Aucun compte actif à cette adresse email"           |>
          { (email, expected) =>
    ...
    "Use token with success"  in new WithBrowser(webDriver = WebDriverFactory(HTMLUNIT), app = applicationWithBrowser) {
      ...
      val loginURL = controllers.routes.LoginController.temporary_redirect().absoluteURL(false, s"localhost:$port")
