...
class ApplicationSpec extends Specification with Tables with BaseSpec {
  ...
  "Application" should {
    "Create Application with success"  in new WithBrowser(webDriver = WebDriverFactory(HTMLUNIT), app = applicationWithBrowser) {
      ...
      val instructorGroup = UserGroup(id = UUIDHelper.randomUUID, name = s"Group $number", description = None, inseeCode = List("0"), creationDate = Time.now(), createByUserId = UUIDHelper.namedFrom("julien"), areaIds = area)
